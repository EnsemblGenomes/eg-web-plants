...
#	$ret = sprintf ("%.0f\%", $1 / $self->{_isearch}->{ServerCount} * 100);
	...
	$ret = $self->{_isearch}->{ServerCount} ? sprintf ("%d", $1 / $self->{_isearch}->{ServerCount} * 100) : 0;
    } else {
	warn "RES2 : [$str]\n";
    }
    return $ret;
...
#		warn Dumper $a;
	    ...
	    if ($self->collection == 23) { # Temp special case for wheat

# we only want alignments of wheat
		next unless $a->{Organism} =~ /Triticum.aestivum/;
# now we need to check if the found alignment is present in the ensembl db
	        my $dba = $self->{_dbc}->get_DBAdaptor('core', 'Triticum_aestivum');
	        my $sa = $dba->get_SliceAdaptor;
		my ($tstart, $tend) = ($a->{targetStart},  $a->{targetEnd});
                ($tstart, $tend ) = ($tend, $tstart) if ($tstart > $tend);
		my $region =  $a->{Accession};

# format of the response is different from EG domains and EMBL domain ... 
		$a->{Accession} = join ':', $a->{Organism}, $region;
