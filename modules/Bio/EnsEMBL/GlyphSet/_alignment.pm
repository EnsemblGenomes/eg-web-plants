  ...
  foreach my $feature_key (@sorted) {
    ...
    foreach my $f (
     ...
     my $hstrand  = $f->can('hstrand')  ? $f->hstrand : 1;
      ...
      my $fgroup_name = $f->can('hseqname') ?  $f->hseqname : $self->feature_group( $f );
      # make all fgroup_names unique, because we don't want to group things by hit names
      while(exists $id{$fgroup_name . ".$fgroup_count"}){$fgroup_count++;}
